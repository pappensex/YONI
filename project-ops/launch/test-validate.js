...
try {
  ...
  log('❌ Test 2 failed: Invalid JSON should have exited with non-zero code', 'red');
  testsFailed++;
} catch (error) {
  if (error.status === 1) {
...
try {
  ...
  fs.writeFileSync(templatePath, JSON.stringify(invalidTemplate, null, 2), 'utf8');
  ...
  if (result.includes('Missing required keys')) {
    ...
    log('⚠️  Test 3: Missing fields were not detected (may have been auto-fixed)', 'yellow');
    testsPassed++;
  }
} catch (error) {
